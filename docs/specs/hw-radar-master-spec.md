...
| 0.23 | 2026-09-26 | Claude (owner decisions, docs reconciliation) | §21 reconciliation only, no new requirement ID: OQ-024 row flipped **Open (part b) → Resolved** — no production Actor-backed merchant source now, F5b deferred and not a blocker ([OQ24](../resolved-questions.md#oq24--production-actor-backed-merchant-source-admission)); the §21 lead-in and References question-record pointer updated to match. The owner's same-dated [OQ31](../resolved-questions.md#oq31--existing-local-connectors-whose-terms-prohibit-automated-access)/[OQ32](../resolved-questions.md#oq32--ms-1e-ratification-gate-when-two-named-sources-are-unusable) decisions (ServerPartDeals/Seagate-recertified retirement, the source × category admission matrix replacing the SA-004 enable order, and the ≥3-source ratification floor) are recorded in `docs/resolved-questions.md` and `docs/handoff/deployed.md`, not as new spec rows. |
...
| OQ-024 (repo [OQ24](../resolved-questions.md#oq24--production-actor-backed-merchant-source-admission)) | Which merchant source may be collected through a Hardware Radar Actor in production? | **Split 2026-09-24, part (b) resolved 2026-09-26:** part (a), the first Actor proof, is resolved — a controlled synthetic source through a real private Actor ([resolved record](../resolved-questions.md#oq24-part-a--first-actor-proof-uses-a-controlled-synthetic-source)); part (b) — no production Actor-backed merchant source is introduced now; F5b is deferred until a future source-admission review finds an eligible candidate; Newegg excluded | No — F5b does not block F6 or MS-2 exit ([OQ28](../resolved-questions.md#oq28--can-ms-2-exit-on-the-synthetic-proof-alone)) | Owner | MS-2 task F5b | **Resolved** |
...
- **Question record:** [`docs/open-questions.md`](../open-questions.md) (no MS-2 session-2 question remains open in this table) · [`docs/resolved-questions.md`](../resolved-questions.md) (settled provenance through OQ32).
