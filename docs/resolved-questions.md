...
- **Verification (orchestrator, no value exposed):** the operator key's `GET /v2/users/me` and `/users/me/limits` succeed (account l3digital, STARTER, current cycle $0.09 of $19); `POST /v2/acts` with an invalid body reaches schema validation, confirming create permission and therefore unscoped status. sha comparison confirms the key is distinct from the `apify-actors` agent token.
...
**✅ Resolved (owner, 2026-09-25) — recorded here; [ADR 0021's 2026-09-25 amendment](adr/adr-0021-hybrid-acquisition-apify.md#amendment--2026-09-25-credential-namespace-runtimeoperator-authority-split-and-policy-values-owner-clarification) carries the architectural rule.** Raised by the MS-2 session-2 review (R33); live paid admission, including the Slice D synthetic proof, is denied while this bound is unset (MS2-D-40 check 2 fails closed).
