...
| FR-014 | Slice B typed satellites (`tests/db/test_category_specs.py`) + Slice C evaluator (`tests/unit/test_eligibility_aggregate.py`, `tests/db/test_watch_evaluation.py`): `match/no_match/unknown` per category; hard unknown/contradictory fields never pass; `EVALUATOR_VERSION` `ms2c.2`. Category-dispatch seam preserves prior drive-matcher decisions via the A0 snapshot oracle | Verified (dev, not deployed) — Slice B+C code-complete on `dev`; production unchanged |
| IR-008 / DR-011 | Apify provider evidence contract, completeness gate, and truncated/partial/failed-run delist veto implemented and tested (`5f37351`, `ef9636c`). D-prep D1 (Actor project `actors/hw-radar-synthetic-collector`, contract schemas, `classify_run`, CI Actor gates) and D3 (client) landed (`990a6d0`/`6b4b000`/`62e670b`/`7680cf6`); no Apify push/build/run has occurred | Partially verified (Slice A seam + D-prep D1/D3 code-complete; live adapter run pending Slice D) |
| NFR-006 | Slice B satellites add no spine-model field change; `makemigrations --check` clean at each Slice B commit | Verified (dev, not deployed) — `tests/db/test_category_specs.py` |
| IR-007 | Slice B refdata importer rejects a category-reference row lacking `source_url`/`retrieved_on`; enforced for the CPU 9 / GPU 8 / RAM 2 first-party seed rows | Verified (dev, not deployed) — `tests/db/test_refdata_categories.py` |
| AC-1 | GPU/RAM/CPU exact-alias hits reach `review` with auto-accept off; cross-category hits never auto-accept; drive baseline unchanged (Slice B) | Verified (dev, not deployed) — `tests/db/test_category_specs.py`, `tests/unit/test_{gpu,ram,cpu}_rules.py` |
| FR-006 | Every `watch_evaluation` row carries non-empty `reasons` (Slice C evaluator) | Verified (dev, not deployed) — `tests/db/test_watch_evaluation.py` |
| DR-004 | Eligibility evidence persisted on `watch_evaluation` (Slice C schema, migration 0020) | Verified (dev, not deployed) — `tests/db/test_watch_evaluation.py` |
| AC-2 | `match/no_match/unknown` per first-class category, parametrized 4 categories × 3 verdicts (Slice C evaluator, C3) | Verified (dev, not deployed) — `tests/db/test_watch_evaluation.py` |
| AC-3 | Shortlist runs on fixture observations with no ADR-0011 scoring artifact; `grep` guard asserts no scoring-module import. Code half only; owner-gated live evidence (F6) not yet run | Partially verified (code half, dev, not deployed) — `tests/unit/test_shortlist.py` |
...
7. Build, version, test, and deploy one self-owned private Hardware Radar Actor from this repository (`actors/`), first proven on a controlled synthetic source ([ADR 0021 amendment 2026-09-24](../adr/adr-0021-hybrid-acquisition-apify.md#amendment--2026-09-24-actor-ownership-billing-cycle-budget-and-actor-proof-owner-clarification)); Actor output is observation-only and carries no hw-radar DB credentials/model imports. A production Actor-backed merchant source is a separate source-admission decision ([OQ24](../open-questions.md#oq24--production-actor-backed-merchant-source-admission)).
...
| OQ-024 (repo [OQ24](../open-questions.md#oq24--production-actor-backed-merchant-source-admission)) | Which merchant source may be collected through a Hardware Radar Actor in production? | **Split 2026-09-24:** part (a), the first Actor proof, is resolved — a controlled synthetic source through a real private Actor ([resolved record](../resolved-questions.md#oq24-part-a--first-actor-proof-uses-a-controlled-synthetic-source)); part (b) stays open — per-candidate source admission (`eligible \| permission-required \| exclude`); Newegg excluded | No (for MS-2 code); yes for an Actor-backed pilot source | Owner | MS-2 task F5b | **Open (part b)** |
...
- **Question record:** [`docs/open-questions.md`](../open-questions.md) (OQ24 open — next is OQ25) · [`docs/resolved-questions.md`](../resolved-questions.md) (settled provenance through OQ23 and OQ24 part a).
