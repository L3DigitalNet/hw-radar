...
- Bounded-retention expiry, eBay delete-on-delist (CR-004), and the per-lane scheduling-state
  split are deployed (migrations 0014-0017, 2026-09-06). Source go-live is gated by the per-cell
  operator checklist (`docs/handoff/deployed.md`) + MS-1e ratification.
...
- MS-1e: the bounded live harvest and draft labels are done (2026-09-25; eBay 29, WD 75,
  goHardDrive 21; ServerPartDeals/Seagate not harvested — retired, OQ31). Provisional gate: FAIL
  (17/100). **OQ32 (owner, 2026-09-26):** the five-source floor is replaced by a metadata-declared
  ≥3-source floor (eBay/WD/goHardDrive; `ratification_sources`), evaluated against production
  refdata. Owner audit + ADR-0019 decision remain. Packet: `docs/evidence/2026-09-25-ms1e-audit-packet.md`.
- **MS-2 plan** (`docs/superpowers/plans/2026-09-24-ms2-multi-category-watch-core.md`) is at
  revision 12 plus landing notes; review lineage in `docs/handoff/specs-plans.md`. **Slices A–C:**
  seams, first-class GPU/RAM/CPU categories (0018-0019, first-party seeds CPU 9 / GPU 8 / RAM 2),
  eligibility + shortlist read model (0020, `EVALUATOR_VERSION` `ms2c.2`). **Deployed 2026-09-25**
  (bug 001 fixed); evidence in `docs/handoff/deployed.md`.
...
- **F-01 (owner-approved):** `HW_RADAR_APIFY_MAX_KV_WRITES` is 3; admission denies values below the
  two-write floor (`f8b8067`). **F1–F4 deployed 2026-09-25** (`c2adae0`, PR #36; sources off, so
  nothing runs): eBay GPU/RAM/CPU category sweeps with per-scope absence; only single-page sweeps
  prove complete; category IDs and the 5,000/day quota re-verified live. F2: no change (ServerPartDeals'
  terms prohibit it, now also retired by OQ31). `pilot_report` added; the scratch-DB pilot found 0%
  condition coverage, unseeded category catalogs, and two connector defects (fixed). F4: 1,888
  unlabeled eBay entries in git-ignored staging. Evidence: `docs/evidence/2026-09-25-f1-f3-pilot.md`.
...
- **Owner decisions:** session 2 (2026-09-24) — HR Actors live in this repo under `actors/`;
  ...
  2026-09-26 — OQ31/OQ32 (retirement + admission matrix + ratification floor, above); OQ24 part
  (b) (no production merchant source now; F5b deferred, not a blocker); **matcher `2026.09.2`**
  (comparison-phrase MPNs masked; Seagate `nm` no longer implies Exos; WD SKU exposes `mpn`).
  Open: MS-1e owner audit, category corpus gate. See `docs/resolved-questions.md`.
