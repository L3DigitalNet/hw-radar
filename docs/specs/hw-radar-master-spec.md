...
| FR-014 | MS-2 category fixtures + DB integration prove `match/no_match/unknown`; hard unknown/contradictory fields never pass. Category-dispatch seam (`tests/` resolver dispatch suite) preserves prior drive-matcher decisions via the A0 snapshot oracle | Partially verified (Slice A dispatch seam; full requirement pending Slice B typed specs) |
| IR-008 / DR-011 | Apify provider integration tests: bounded run metadata, idempotent duplicate import, provider-switch identity preservation, truncated-run delist veto, budget fail-closed. Provider run-evidence contract, completeness gate, and truncated/partial/failed-run delist veto implemented and tested (`5f37351`, `ef9636c`) | Partially verified (Slice A provider-evidence/completeness seam; Apify adapter itself pending Slice D) |
